...
**Image attributes (EAV):** `image_attributes(asset_id, key, value)` is a vertical entity-attribute-value store — new per-image data becomes new rows, not new columns, so tracking another attribute never needs a schema migration (it's a brand-new table, so `CREATE TABLE IF NOT EXISTS` in `SCHEMA_SQL` adds it to existing shards on open). Booleans are presence-style: value `'1'` when true, row absent when false (`imgdb.set_image_flag`/`get_image_flag`); the `(key, value)` index serves filters. Filter fields (`filter_model`) are correlated subqueries over `all_image_attributes` — the same cost profile as `tag_count` et al. Current keys: `ATTR_IS_FAVORITE` (boolean, toggled from the detail panel), `ATTR_SCAN_AT` (timestamp, set by `_record_scan_time` in the scan's final transaction: stamps exactly that scan's `new_ids` with one shared `'YYYY-MM-DD HH:MM:SS'` UTC value, leaving earlier scans' stamps intact so the history accumulates). Replicate this table per object type (datasets, roots) when needed — images only for now.

**Non-boolean EAV attributes:** `scan_at` is the pattern for a value-carrying attribute. It appears twice in `filter_model`: as a `FilterField` of dtype `timestamp` (ISO-8601 sorts and compares lexicographically, so `DTYPE_OPS["timestamp"]` reuses the integer comparison set and `build_filter_conditions` handles it in the same branch as text/integer, binding the value as a string) and as a `SortField`. `SortField.sql_expr` therefore accepts a full `{alias}`-templated expression, not just a bare column — `build_sort_clause(rules, alias)` substitutes it exactly like `FilterField.sql_expr`. `ui_filter_panel` only appends the `(column)` hint to a sort field's label when `sql_expr.isidentifier()`. Note that rows with no stamp (assets ingested before the attribute existed) have a NULL subquery result and so drop out of `!=`/`<`/`>` comparisons — standard SQL NULL semantics, not a bug.
